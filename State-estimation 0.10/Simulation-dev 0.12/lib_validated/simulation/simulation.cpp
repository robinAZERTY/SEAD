...
void simulation::add_sensor(sensor &new_sensor)
{
    ...
    if (t == typeid(accelerometer) | t == typeid(gyroscope) | t == typeid(magnetometer))
    {
...
void simulation::set_current_frame_data()
{
    ...
    {   
        ...
        {   
            ...
            sensor_data[i] = acc.measure(scenario->get_state(), time);
        }
        else if (t == typeid(gyroscope))
        {
            gyroscope &gyro = dynamic_cast<gyroscope &>(*sensors[i]);
            sensor_data[i] = gyro.measure(scenario->get_state(), time);
        }
        else if (t == typeid(magnetometer))
        {
            magnetometer &mag = dynamic_cast<magnetometer &>(*sensors[i]);
            sensor_data[i] = mag.measure(scenario->get_state(), time);
        }
    }
